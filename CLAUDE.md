...
**arXiv is the primary source of truth** for publication metadata (authors, journal-ref, DOI). The owner does not want to maintain `content/About/papers.bib` manually.
...
3. **Identity check — critical:** `content/About/papers.bib` lists known papers by this owner. Before including any arXiv result, verify it overlaps with the bib file (same title or arXiv ID). Do not include papers by other authors who happen to share the name "Wangqian Miao" or "Miao W".
